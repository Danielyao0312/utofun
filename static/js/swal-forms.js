  ...
  function Input (field) {
    var input = {
      ...
      toHtml: function () {
        var inputTag
        if (input.type == 'text') {
          inputTag = t("<br/><label class='{clazz} swal-form-field' for='{name}'>{value}</label>", input);
        }
        else if (input.type == 'input' || input.type == 'password') {
          ...
          inputTag = t("<button id='{id}'>{value}</button>", input)
        }
        return inputTag 
